...
class UserCredentialStore:
    ...
    def __init__(self, redis_db: Optional[int] = None):
        ...
        if redis_db and redis_db != redis_service._db:
            ...
            self.redis_client = RedisClient(db=redis_db or settings.REDIS_DB_CREDENTIALS)
        else:
    ...
    async def store_credentials(self, user_id: str, credentials: Credentials) -> None:
        ...
        # Store without expiration - credentials persist until explicitly revoked
        await self.redis_client.set(key, credentials_json)
