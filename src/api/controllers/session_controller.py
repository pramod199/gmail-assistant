...
from ...config.settings import MAX_CONCURRENT_SESSIONS_PER_USER
import logging
    ...
    **Next Steps:**
    ...
    try:
        ...
        if not gmail_authorized:
            ...
            from ...api.controllers.auth_controller import _generate_auth_url
        ...
        if active_sessions >= MAX_CONCURRENT_SESSIONS_PER_USER:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail={
                    "error": "too_many_sessions",
                    "message": f"Maximum concurrent sessions limit ({MAX_CONCURRENT_SESSIONS_PER_USER}) reached. Please close an existing session first.",
                    "active_sessions": active_sessions,
                    "max_allowed": MAX_CONCURRENT_SESSIONS_PER_USER
                }
            )
